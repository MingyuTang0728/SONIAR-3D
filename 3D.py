...
# 修复：确保 base64 字符串没有换行符，防止破坏 JS 字符串
logo_src = f"data:image/png;base64,{logo_base64.replace(chr(10), '')}" if logo_base64 else ""

# ==========================================
# HTML/JS 核心代码
# 使用纯字符串拼接，避免任何 f-string 语法冲突
# ==========================================
html_template = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .jump-text { color: var(--neon); font-size: 3rem; font-weight: bold; margin-bottom: 20px; letter-spacing: 0.2em; text-align: center; }
        
        /* 显式跳转按钮 (Fallback) */
        #manual-jump-btn {
            background: var(--neon); color: black; font-family: 'Orbitron'; font-weight: bold;
            padding: 15px 40px; font-size: 1.2rem; border: none; cursor: pointer;
            box-shadow: 0 0 20px var(--neon); border-radius: 5px;
            text-decoration: none; display: inline-block;
            transition: transform 0.2s;
        }
        #manual-jump-btn:hover { transform: scale(1.1); background: white; }
    </style>
</head>
<body>
...
<div id="jump-overlay">
    <div class="jump-text">WARPING INITIATED</div>
    <a id="manual-jump-btn" href="__TARGET_URL__" target="_blank">ENTER UK HUB</a>
    <div style="color: gray; margin-top: 10px; font-size: 0.8rem;">(Click if not redirected)</div>
</div>
...
<script type="module">
    ...
    const UKMarker = ({ isHovered, pinchProgress }) => {
        ...
        // 修复：将样式对象提取到外部，避免解析错误
        const htmlStyle = { pointerEvents: 'none' };
        ...
        return html`
            <group position=${[0, 0, 0]}>
                ...
                <${Html} position=${[0, 0.8, 0]} center style=${htmlStyle}>
    ...
    const Earth = ({ targetRotation, onHoverUK, pinchProgress, isExploding }) => {
        ...
        useFrame(() => {
            ...
            if (isExploding && earthRef.current) {
                earthRef.current.scale.multiplyScalar(1.02); // 减缓爆炸速度防止渲染崩溃
                if (earthRef.current.material.opacity > 0) {
                   earthRef.current.material.opacity -= 0.02;
                ...
                if (landmarker && video.readyState >= 2) {
                    ...
                    if (results.landmarks && results.landmarks.length > 0) {
                        ...
                        if (hitUK && isPinching) {
                            ...
                            const progress = Math.min((Date.now() - pinchStart) / 1500, 1); 
                            ...
                            if (progress >= 1) {
                                ...
                                // 尝试自动点击按钮（比 window.open 安全一点）
                                setTimeout(() => {
                                    try {
                                        const btn = document.getElementById('manual-jump-btn');
                                        if(btn) btn.click();
                                        // 备选方案：如果自动点击被拦截，window.open 尝试
                                        else window.open(TARGET_URL, '_blank');
                                    } catch(e) {
                                        console.log("Auto-jump blocked, user must click button.");
                                    }
                                }, 1000);
                            }
                        } else {
    ...
    const App = () => {
        ...
        // 样式对象单独定义，防止 Python 解析错误
        const canvasStyle = { width: '100vw', height: '100vh', background: 'black' };
